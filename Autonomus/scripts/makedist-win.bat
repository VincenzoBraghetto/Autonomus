@echo off

REM - batch file to build Visual Studio project and zip the resulting binaries (or make installer)
REM - updating version numbers requires python and python path added to %PATH% env variable 
REM - zipping requires 7zip in %ProgramFiles%\7-Zip\7z.exe
REM - building installer requires innosetup 6 in "%ProgramFiles(x86)%\Inno Setup 6\iscc"
REM - AAX codesigning requires wraptool tool added to %PATH% env variable and aax.key/.crt in .\..\..\iPlug2\Certificates\

REM - two arguments are demo/full and zip/installer

set DEMO_ARG="%1"
set ZIP_ARG="%2"

if [%DEMO_ARG%]==[] goto USAGE
if [%ZIP_ARG%]==[] goto USAGE

echo SCRIPT VARIABLES -----------------------------------------------------
echo DEMO_ARG %DEMO_ARG% 
echo ZIP_ARG %ZIP_ARG% 
echo END SCRIPT VARIABLES -----------------------------------------------------

if %DEMO_ARG% == "demo" (
  echo Making Autonomus Windows DEMO VERSION distribution ...
  set DEMO=1
) else (
  echo Making Autonomus Windows FULL VERSION distribution ...
  set DEMO=0
)

if %ZIP_ARG% == "zip" (
  set ZIP=1
) else (
  set ZIP=0
)

echo ------------------------------------------------------------------
echo Updating version numbers ...

call python prepare_resources-win.py %DEMO%
@REM call python prepare_installer_docs.py win
@REM if %ERRORLEVEL% neq 0 (
@REM   echo ERROR: preparing installer docs failed
@REM   exit /B 1
@REM )
@REM call python update_installer-win.py %DEMO%

cd ..\

echo "touching source"

copy /b *.cpp+,,

echo ------------------------------------------------------------------
echo Building ...

if exist "%ProgramFiles%\Microsoft Visual Studio\2022\Professional\VC\Auxiliary\Build\vcvarsall.bat" (
  call "%ProgramFiles%\Microsoft Visual Studio\2022\Professional\VC\Auxiliary\Build\vcvarsall.bat" x64
) else (
  call "%ProgramFiles%\Microsoft Visual Studio\2022\Community\VC\Auxiliary\Build\vcvarsall.bat" x64
)

REM - set preprocessor macros like this, for instance to set demo preprocessor macro:
if %DEMO% == 1 (
  set CMDLINE_DEFINES="DEMO_VERSION=1"
  REM -copy ".\resources\img\AboutBox_Demo.png" ".\resources\img\AboutBox.png"
) else (
  set CMDLINE_DEFINES="DEMO_VERSION=0"
  REM -copy ".\resources\img\AboutBox_Registered.png" ".\resources\img\AboutBox.png"
)

REM - Could build individual targets like this:
REM - msbuild Autonomus-app.vcxproj /p:configuration=release /p:platform=x64

echo Building x64 binaries...
REM add projects with /t to build VST2 and AAX
msbuild Autonomus.sln /t:Autonomus-app /p:configuration=release /p:platform=x64 /nologo /verbosity:minimal /fileLogger /m /flp:logfile=build-win.log;errorsonly
if %ERRORLEVEL% neq 0 (
  echo ERROR: x64 build failed
  type build-win.log
  exit /B 1
)

@REM echo Building ARM64EC binaries...
@REM msbuild Autonomus.sln /t:Autonomus-app /p:configuration=release /p:platform=ARM64EC /nologo /verbosity:minimal /fileLogger /m /flp:logfile=build-win-arm64ec.log;errorsonly
@REM if %ERRORLEVEL% neq 0 (
@REM   echo ERROR: ARM64EC build failed
@REM   type build-win-arm64ec.log
@REM   exit /B 1
@REM )

REM --echo Copying AAX Presets

REM --echo ------------------------------------------------------------------
REM --echo Code sign AAX binary...
REM --info at pace central, login via iLok license manager https://www.paceap.com/pace-central.html
REM --wraptool sign --verbose --account XXXXX --wcguid XXXXX --keyfile XXXXX.p12 --keypassword XXXXX --in .\build-win\aax\bin\Autonomus.aaxplugin\Contents\Win32\Autonomus.aaxplugin --out .\build-win\aax\bin\Autonomus.aaxplugin\Contents\Win32\Autonomus.aaxplugin
REM --wraptool sign --verbose --account XXXXX --wcguid XXXXX --keyfile XXXXX.p12 --keypassword XXXXX --in .\build-win\aax\bin\Autonomus.aaxplugin\Contents\x64\Autonomus.aaxplugin --out .\build-win\aax\bin\Autonomus.aaxplugin\Contents\x64\Autonomus.aaxplugin

@REM if %ZIP% == 0 (
@REM REM - Make Installer (InnoSetup)

@REM echo ------------------------------------------------------------------
@REM echo Making Installer ...

@REM   REM if exist "%ProgramFiles(x86)%" (goto 64-Bit-is) else (goto 32-Bit-is)

@REM   REM :32-Bit-is
@REM   REM REM "%ProgramFiles%\Inno Setup 6\iscc" /Q ".\installer\Autonomus.iss"
@REM   REM goto END-is

@REM   REM :64-Bit-is
@REM   "%ProgramFiles(x86)%\Inno Setup 6\iscc" /Q ".\installer\Autonomus.iss"
@REM   if %ERRORLEVEL% neq 0 (
@REM     echo ERROR: installer build failed
@REM     exit /B 1
@REM   )
@REM   REM goto END-is

@REM   REM :END-is

@REM   REM - Codesign Installer for Windows 8+
@REM   REM -"C:\Program Files (x86)\Microsoft SDKs\Windows\v7.1A\Bin\signtool.exe" sign /f "XXXXX.p12" /p XXXXX /d "Autonomus Installer" ".\installer\Autonomus Installer.exe"

@REM   REM -if %1 == 1 (
@REM   REM -copy ".\installer\Autonomus Installer.exe" ".\installer\Autonomus Demo Installer.exe"
@REM   REM -del ".\installer\Autonomus Installer.exe"
@REM   REM -)

@REM   echo Making Zip File of Installer ...
@REM ) else (
@REM   echo Making Zip File ...
@REM )

FOR /F "tokens=* USEBACKQ" %%F IN (`call python scripts\makezip-win.py %DEMO% %ZIP%`) DO (
SET ZIP_NAME=%%F
)

echo ------------------------------------------------------------------
echo Printing log file to console...

type build-win.log
goto SUCCESS

:USAGE
echo Usage: %0 [demo/full] [zip/installer]
exit /B 1

:SUCCESS
echo %ZIP_NAME%

exit /B 0
