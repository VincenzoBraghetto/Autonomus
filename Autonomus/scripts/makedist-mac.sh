...
NOTARIZE_BUNDLE_ID=com.VincenzoBraghetto.Autonomus
NOTARIZE_BUNDLE_ID_DEMO=com.VincenzoBraghetto.Autonomus.DEMO
...
if [ "$USE_XCPRETTY" -eq "1" ]; then
  ...
  xcodebuild -project ./projects/$PLUGIN_NAME-macOS.xcodeproj -xcconfig ./config/$PLUGIN_NAME-mac.xcconfig DEMO_VERSION=$DEMO -target "APP" -configuration Release
  if [ "$?" -ne "0" ]; then
    echo "ERROR: build failed, aborting"
    exit 1
  fi
...
if [ $BUILD_INSTALLER == 1 ]; then
  ...
  cp $GITHUB_WORKSPACE/LICENSE.txt build-mac/zip/
...
#if [ $DEMO == 1 ]
#then
#  git checkout installer/Autonomus.iss
#  git checkout installer/Autonomus.pkgproj
